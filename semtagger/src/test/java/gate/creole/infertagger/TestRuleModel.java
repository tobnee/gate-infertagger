...
public class TestRuleModel {
	...
	public void testToken() {
		Token t1 = RuleModelFactory.fromToken(annoTok);
		assertNotNull(t1);
		...
		assertEquals("default",
				t1.getFeatureOrDefault("noneExsisting", "default"));

		Word w1 = RuleModelFactory.fromWordToken(annoTok);
		Token w2 = RuleModelFactory.fromWordToken(annoTok2);
	...
	public void testSentence() {
		Token t1 = RuleModelFactory.fromToken(annoTok);
		Sentence sentence = RuleModelFactory.fromSentence(annoSen);
	...
	public void testLookup() {
		Sentence sen = RuleModelFactory.fromSentence(annoSen);
		Lookup look = RuleModelFactory.fromLookup(annoLook);
		assertNotNull(look);
		look.within(sen);
	}
	...
	public void before() {
		...
		annoTok = (AnnotationImpl) annoFac.createAnnotationInSet(annoSet, 0,
		...
		t2Tok.put("kind", "word");
		annoTok2 = (AnnotationImpl) annoFac.createAnnotationInSet(annoSet, 0,
				new NodeImpl(0, 7l), new NodeImpl(1, 12l), "Token", t2Tok);

		FeatureMap tok = Factory.newFeatureMap();
		annoLook = (AnnotationImpl) annoFac.createAnnotationInSet(annoSet, 0,
